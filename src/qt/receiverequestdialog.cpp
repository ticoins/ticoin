...
#include "ticoin-config.h" /* for USE_QRCODE */
#endif
...
void ReceiveRequestDialog::setModel(OptionsModel *model)
{
    ...
    /**-1-12update the display unit if necessary
    update();
}
...
void ReceiveRequestDialog::update()
{
    ...
    QString uri = GUIUtil::formatticoinURI(info);
    ...
    if(info.amount)
        html += "<b>"+tr("Amount")+"</b>: " + ticoinUnits::formatWithUnit(model->getDisplayUnit(), info.amount) + "<br>";
...
{
    GUIUtil::setClipboard(GUIUtil::formatticoinURI(info));
}
