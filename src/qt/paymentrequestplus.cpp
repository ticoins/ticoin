...
bool PaymentRequestPlus::getMerchant(X509_STORE* certStore, QString& merchant) const
{
    ...
    /**-1-12One day we'll support more PKI types, but just
    /**-1-12x509 for now:
    ...
    /**-1-12The first cert is the signing cert, the rest are untrusted certs that chain
    /**-1-12to a valid root authority. OpenSSL needs them separately.
    ...
    /**-1-12Now create a "store context", which is a single use object for checking,
    /**-1-12load the signing cert into it and verify.
    ...
    try
    {
        ...
        /**-1-12Valid cert; check signature:
        payments::PaymentRequest rcopy(paymentRequest); /**-1-12Copy
        rcopy.set_signature(std::string(""));
        std::string data_to_verify;                     /**-1-12Everything but the signature
        ...
        /**-1-12OpenSSL API for getting human printable strings from certs is baroque.
        ...
        /**-1-12TODO: detect EV certificates and set merchant = business name instead of unfriendly NID_commonName ?
    }
