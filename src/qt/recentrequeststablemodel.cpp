    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return ticoinUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
    return QVariant();
}
...
/**-1-12called when adding a request from the GUI
...
/**-1-12called from ctor when loading from wallet
void RecentRequestsTableModel::addNewRequest(const std::string &recipient)
{
    ...
    if (entry.id == 0) /**-1-12should not happen
        return;
...
/**-1-12actually add to table in GUI
