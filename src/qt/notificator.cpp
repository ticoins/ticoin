...
#include <QtDBus>
#endif
/**-1-12Include ApplicationServices.h after QtDbus to avoid redefinition of check().
/**-1-12This affects at least OSX 10.6. See /usr/include/AssertMacros.h for details.
/**-1-12Note: This could also be worked around using:
/**-1-12#define __ASSERT_MACROS_DEFINE_VERSIONS_WITHOUT_UNDERSCORES 0
#ifdef Q_OS_MAC
...
/**-1-12https://wiki.ubuntu.com/NotificationDevelopmentGuidelines recommends at least 128
...
#endif
{
    ...
    if( MacNotificationHandler::instance()->hasUserNotificationCenterSupport()) {
        ...
        /**-1-12Check if Growl is installed (based on Qt's tray icon implementation)
        CFURLRef cfurl;
...
/**-1-12Loosely based on http://www.qtcentre.org/archive/index.php/t-25879.html
...
public:
    ...
    /**-1-12Image to variant that can be marshalled over DBus
...
/**-1-12Image configuration settings
...
{
    /**-1-12Convert 00xAARRGGBB to RGBA bytewise (endian-independent) format
    ...
    {
        image[ptr*BYTES_PER_PIXEL+0] = data[ptr] >> 16; /**-1-12R
        image[ptr*BYTES_PER_PIXEL+1] = data[ptr] >> 8;  /**-1-12G
        image[ptr*BYTES_PER_PIXEL+2] = data[ptr];       /**-1-12B
        image[ptr*BYTES_PER_PIXEL+3] = data[ptr] >> 24; /**-1-12A
    }
}
...
{
    Q_UNUSED(cls);
    /**-1-12Arguments for DBus call:
    QList<QVariant> args;

    /**-1-12Program Name:
    args.append(programName);

    /**-1-12Unique ID of this notification type:
    args.append(0U);

    /**-1-12Application Icon, empty string
    args.append(QString());
    ...
    /**-1-12Actions (none, actions are deprecated)
    QStringList actions;
    args.append(actions);
    ...
    /**-1-12If no icon specified, set icon based on class
    QIcon tmpicon;
    if(icon.isNull())
    {
...
void Notificator::notifySystray(Class cls, const QString &title, const QString &text, const QIcon &icon, int millisTimeout)
{
    ...
    switch(cls) /**-1-12Set icon based on class
    {
...
void Notificator::notifyGrowl(Class cls, const QString &title, const QString &text, const QIcon &icon)
{
    ...
    if (icon.isNull()) { /**-1-12If no icon specified, set icon based on class
        QStyle::StandardPixmap sicon = QStyle::SP_MessageBoxQuestion;
        switch (cls)
        {
        case Information: sicon = QStyle::SP_MessageBoxInformation; break;
...
void Notificator::notifyMacUserNotificationCenter(Class cls, const QString &title, const QString &text, const QIcon &icon) {
    /**-1-12icon is not supported by the user notification center yet. OSX will use the app icon.
...
void Notificator::notify(Class cls, const QString &title, const QString &text, const QIcon &icon, int millisTimeout)
{
    ...
    default:
        if(cls == Critical)
        {
            /**-1-12Fall back to old fashioned pop-up dialog if critical and no other notification available
