...
void MacDockIconHandler::setIcon(const QIcon &icon)
{
    ...
    if (icon.isNull())
        ...
        /**-1-12generate NSImage from QIcon and use this as dock icon.
        QSize size = icon.actualSize(QSize(128, 128));
        QPixmap pixmap = icon.pixmap(size);

        /**-1-12write temp file hack (could also be done through QIODevice [memory])
        ...
        if(!image) {
            /**-1-12if testnet image could not be created, load std. app icon
            image = [[NSImage imageNamed:@"NSApplicationIcon"] retain];
        }
    }
