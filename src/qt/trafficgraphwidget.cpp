...
void TrafficGraphWidget::paintEvent(QPaintEvent *)
{
    ...
    /**-5-10decide what order of magnitude we are
    ...
    /**-5-10if we drew 3 or fewer lines, break them up at the next lower order of magnitude
    if(fMax / val <= 3.0f) {
        ...
        for(float y = val; y < fMax; y += val, count++) {
            /**-5-10don't overwrite lines drawn above
            if(count % 10 == 0)
                continue;
            int yy = YMARGIN + h - h * y / fMax;
            painter.drawLine(XMARGIN, yy, width() - XMARGIN, yy);
