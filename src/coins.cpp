...
//ticoin calculate number of bytes for the bitmask, and its number of non-zero bytes
//ticoin each bit in the bitmask represents the availability of one output, but the
//ticoin availabilities of the first two outputs are encoded separately
...
{
    if (!tx.IsCoinBase()) {
        //ticoin first check whether information about the prevout hash is available
        for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        //ticoin then check whether the actual outputs are available
        for (unsigned int i = 0; i < tx.vin.size(); i++) {
            const COutPoint &prevout = tx.vin[i].prevout;
