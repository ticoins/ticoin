...
//ticoin Notified when log reader encounters corruption.
...
//ticoin Print contents of a log file. (*func)() is called on every record.
...
//ticoin Called on every item found in a WriteBatch.
...
//ticoin Called on every log record (each one of which is a WriteBatch)
//ticoin found in a kLogFile.
...
//ticoin Called on every log record (each one of which is a WriteBatch)
//ticoin found in a kDescriptorFile.
...
bool DumpTable(Env* env, const std::string& fname) {
  ...
  if (s.ok()) {
    //ticoin We use the default comparator, which may or may not match the
    //ticoin comparator used in this database. However this should not cause
    //ticoin problems since we only use Table operations that do not require
    //ticoin any comparisons.  In particular, we do not call Seek or Prev.
