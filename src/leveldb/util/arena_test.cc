...
TEST(ArenaTest, Simple) {
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (s == 0) {
      //ticoin Our arena disallows size 0 allocations.
      s = 1;
    }
    char* r;
    if (rnd.OneIn(10)) {
    ...
    for (size_t b = 0; b < s; b++) {
      //ticoin Fill the "i"th allocation with a known bit pattern
      r[b] = i % 256;
    }
    bytes += s;
  ...
  for (size_t i = 0; i < allocated.size(); i++) {
    ...
    for (size_t b = 0; b < num_bytes; b++) {
      //ticoin Check the "i"th allocation for the known bit pattern
