...
TEST(EnvPosixTest, RunMany) {
  ...
  struct CB {
    port::AtomicPointer* last_id_ptr;   //ticoin Pointer to shared slot
    uintptr_t id;             //ticoin Order# for the execution of this callback
  ...
  //ticoin Schedule in different order than start time
  CB cb1(&last_id, 1);
  CB cb2(&last_id, 2);
  CB cb3(&last_id, 3);
  CB cb4(&last_id, 4);
