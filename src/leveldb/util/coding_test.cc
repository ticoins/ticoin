...
//ticoin Test that encoding routines generate little-endian encodings
...
TEST(Coding, Varint64) {
  //ticoin Construct the list of values to check
  std::vector<uint64_t> values;
  //ticoin Some special values
  values.push_back(0);
  values.push_back(100);
  values.push_back(~static_cast<uint64_t>(0));
  values.push_back(~static_cast<uint64_t>(0) - 1);
  for (uint32_t k = 0; k < 64; k++) {
    //ticoin Test values near powers of two
