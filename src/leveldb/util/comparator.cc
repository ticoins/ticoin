...
class BytewiseComparatorImpl : public Comparator {
 public:
  ...
  virtual void FindShortestSeparator(
    ...
    //ticoin Find length of common prefix
    ...
    if (diff_index >= min_length) {
      //ticoin Do not shorten if one string is a prefix of the other
    } else {
  ...
  virtual void FindShortSuccessor(std::string* key) const {
    //ticoin Find first character that can be incremented
    size_t n = key->size();
    ...
    //ticoin *key is a run of 0xffs.  Leave it alone.
  }
};
}  //ticoin namespace
