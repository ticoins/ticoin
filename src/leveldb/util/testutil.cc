...
Slice RandomString(Random* rnd, int len, std::string* dst) {
  dst->resize(len);
  for (int i = 0; i < len; i++) {
    (*dst)[i] = static_cast<char>(' ' + rnd->Uniform(95));   //ticoin ' ' .. '~'
  }
  return Slice(*dst);
}

std::string RandomKey(Random* rnd, int len) {
  //ticoin Make sure to generate a wide variety of characters so we
  //ticoin test the boundary conditions for short-key optimizations.
...
extern Slice CompressibleString(Random* rnd, double compressed_fraction,
  ...
  //ticoin Duplicate the random data until we have filled "len" bytes
  dst->clear();
  while (dst->size() < len) {
    dst->append(raw_data);
  }
