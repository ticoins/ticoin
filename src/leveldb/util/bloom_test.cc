...
TEST(BloomTest, VaryingLengths) {
  ...
  //ticoin Count number of filters that significantly exceed the false positive rate
  ...
  for (int length = 1; length <= 10000; length = NextLength(length)) {
    ...
    //ticoin All added keys must match
    for (int i = 0; i < length; i++) {
      ASSERT_TRUE(Matches(Key(i, buffer)))
          << "Length " << length << "; key " << i;
    }

    //ticoin Check false positive rate
    double rate = FalsePositiveRate();
    if (kVerbose >= 1) {
      fprintf(stderr, "False positives: %5.2f%% @ length = %6d ; bytes = %6d\n",
              rate*100.0, length, static_cast<int>(FilterSize()));
    }
    ASSERT_LE(rate, 0.02);   //ticoin Must not be over 2%
    if (rate > 0.0125) mediocre_filters++;  //ticoin Allowed, but not too often
    else good_filters++;
  }
  if (kVerbose >= 1) {
...
//ticoin Different bits-per-byte
