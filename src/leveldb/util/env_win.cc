//ticoin This file contains source that originates from:
//ticoin http://code.google.com/p/leveldbwin/source/browse/trunk/win32_impl_src/env_win32.h
//ticoin http://code.google.com/p/leveldbwin/source/browse/trunk/win32_impl_src/port_win32.cc
//ticoin Those files dont' have any explict license headers but the 
//ticoin project (http://code.google.com/p/leveldbwin/) lists the 'New BSD License'
//ticoin as the license.
...
private:
    ...
    HANDLE _hFile;
    size_t _page_size;
    size_t _map_size;       //ticoin How much extra memory to map at a time
    char* _base;            //ticoin The mapped region
    HANDLE _base_handle;	
    char* _limit;           //ticoin Limit of the mapped region
    char* _dst;             //ticoin Where to write next  (in range [base_,limit_])
    char* _last_sync;       //ticoin Where have we synced up to
    uint64_t _file_offset;  //ticoin Offset of base_ in file
    //LARGE_INTEGER file_offset_;
    //ticoin Have we done an munmap of unsynced data?
    bool _pending_sync;

    //ticoin Roundup x to a multiple of y
...
bool Win32MapFile::_UnmapCurrentRegion()
{
    ...
    if (_base != NULL) {
        if (_last_sync < _limit) {
            //ticoin Defer syncing this data until next Sync() call, if any
            _pending_sync = true;
        }
        ...
        _last_sync = NULL;
        _dst = NULL;
        //ticoin Increase the amount we map the next time, but capped at 1MB
        if (_map_size < (1<<20)) {
            _map_size *= 2;
        }
    }
...
Status Win32MapFile::Close()
{
    ...
    if (!_UnmapCurrentRegion()) {
        ...
        //ticoin Trim the extra space at the end of the file
        LARGE_INTEGER newSize;
...
Status Win32MapFile::Sync()
{
    Status s;
    if (_pending_sync) {
        //ticoin Some unmapped data was not synced
        _pending_sync = false;
    ...
    if (_dst > _last_sync) {
        //ticoin Find the beginnings of the pages that contain the first and last
        //ticoin bytes to be synced.
...
void Win32Logger::Logv( const char* format, va_list ap )
{
    ...
    //ticoin We try twice: the first time with a fixed-size stack allocated buffer,
    //ticoin and the second time with a much larger dynamically allocated buffer.
    char buffer[500];
    for (int iter = 0; iter < 2; iter++) {
        ...
        //ticoin Truncate to available space if necessary
        if (p >= limit) {
            if (iter == 0) {
                continue;       //ticoin Try again with larger buffer
            } else {
                p = limit - 1;
            }
        }

        //ticoin Add newline if necessary
        if (p == base || p[-1] != '\n') {
            *p++ = '\n';
        }
...
static Status CreateDirInner( const std::string& dirname )
{
    ...
    if (attr == INVALID_FILE_ATTRIBUTES) { //ticoin doesn't exist:
...
#endif //ticoin defined(LEVELDB_PLATFORM_WINDOWS)
