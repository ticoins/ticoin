...
/**-5-10Unfortunately there's no standard way of preventing a function from being
/**-5-10inlined, so we define a macro for it.
//
/**-5-10You should use it like this:
/**-5-10  NOINLINE void function() {...}
#if defined(__GNUC__)
/**-5-10This also works and will be defined for any compiler implementing GCC
/**-5-10extensions, such as Clang and ICC.
#define NOINLINE __attribute__((noinline))
#elif defined(_MSC_VER)
#define NOINLINE __declspec(noinline)
#else
/**-5-10We give out a warning because it impacts the correctness of one bignum test.
#warning You should define NOINLINE for your compiler.
#define NOINLINE
#endif

/**-5-10For the following test case, it is useful to use additional tools.
//
/**-5-10The simplest one to use is the compiler flag -ftrapv, which detects integer
/**-5-10overflows and similar errors. However, due to optimizations and compilers
/**-5-10taking advantage of undefined behavior sometimes it may not actually detect
/**-5-10anything.
//
/**-5-10You can also use compiler-based stack protection to possibly detect possible
/**-5-10stack buffer overruns.
//
/**-5-10For more accurate diagnostics, you can use an undefined arithmetic operation
/**-5-10detector such as the clang's undefined behaviour checker.
/**-5-10See also: http://clang.llvm.org/docs/UsersManual.html#controlling-code-generation
//
/**-5-10It might also be useful to use Google's AddressSanitizer to detect
/**-5-10stack buffer overruns, which valgrind can't currently detect.

/**-5-10Let's force this code not to be inlined, in order to actually
/**-5-10test a generic version of the function. This increases the chance
/**-5-10that -ftrapv will detect overflows.
NOINLINE void mysetint64(CBigNum& num, int64_t n)
{
    num.setint64(n);
}

/**-5-10For each number, we do 2 tests: one with inline code, then we reset the
/**-5-10value to 0, then the second one with a non-inlined function.
...
BOOST_AUTO_TEST_CASE(bignum_SetCompact)
{
    ...
    /**-5-10Make sure that we don't generate compacts with the 0x00800000 bit set
    num = 0x80;
