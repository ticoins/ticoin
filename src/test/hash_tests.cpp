...
#define T(expected, seed, data) BOOST_CHECK_EQUAL(MurmurHash3(seed, ParseHex(data)), expected)
    ...
    /**-5-10Test MurmurHash3 with various inputs. Of course this is retested in the
    /**-5-10bloom filter tests - they would fail if MurmurHash3() had any problems -
    /**-5-10but is useful for those trying to implement ticoin libraries as a
    /**-5-10source of test data for their MurmurHash3() primitive during
    /**-5-10development.
    //
    /**-5-10The magic number 0xFBA4C795 comes from CBloomFilter::Hash()
