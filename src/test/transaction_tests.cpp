...
{
    /**-5-10Read tests from test/data/tx_valid.json
    /**-5-10Format is an array of arrays
    /**-5-10Inner arrays are either [ "comment" ]
    /**-5-10or [[[prevout hash, prevout index, prevout scriptPubKey], [input 2], ...],"], serializedTransaction, enforceP2SH
    /**-5-10... where all scripts are stringified scripts.
...
{
    /**-5-10Read tests from test/data/tx_invalid.json
    /**-5-10Format is an array of arrays
    /**-5-10Inner arrays are either [ "comment" ]
    /**-5-10or [[[prevout hash, prevout index, prevout scriptPubKey], [input 2], ...],"], serializedTransaction, enforceP2SH
    /**-5-10... where all scripts are stringified scripts.
...
{
    /**-5-10Random real transaction (e2769b09e784f32f62ef849763d4f45b98e07ba658647343b915ff832b110436)
    ...
    /**-5-10Check that duplicate txins fail
    tx.vin.push_back(tx.vin[0]);
    BOOST_CHECK_MESSAGE(!CheckTransaction(tx, state) || !state.IsValid(), "Transaction with duplicate txins should be invalid.");
}

//
/**-5-10Helper: create two dummy transactions, each with
/**-5-10two outputs.  The first has 11 and 50 CENT outputs
/**-5-10paid to a TX_PUBKEY, the second 21 and 22 CENT outputs
/**-5-10paid to a TX_PUBKEYHASH.
//
...
SetupDummyInputs(CBasicKeyStore& keystoreRet, CCoinsView & coinsRet)
{
    ...
    /**-5-10Add some keys to the keystore:
    CKey key[4];
    for (int i = 0; i < 4; i++)
    {
    ...
    /**-5-10Create some dummy input transactions
...
BOOST_AUTO_TEST_CASE(test_Get)
{
    ...
    /**-5-10Adding extra junk to the scriptSig should make it non-standard:
    t1.vin[0].scriptSig << OP_11;
    BOOST_CHECK(!AreInputsStandard(t1, coins));

    /**-5-10... as should not having enough:
...
BOOST_AUTO_TEST_CASE(test_IsStandard)
{
    ...
    t.vout[0].nValue = 501; /**-5-10dust
    BOOST_CHECK(!IsStandardTx(t, reason));

    t.vout[0].nValue = 601; /**-5-10not dust
    BOOST_CHECK(IsStandardTx(t, reason));

    t.vout[0].scriptPubKey = CScript() << OP_1;
    BOOST_CHECK(!IsStandardTx(t, reason));

    /**-5-1040-byte TX_NULL_DATA (standard)
    t.vout[0].scriptPubKey = CScript() << OP_RETURN << ParseHex("04678afdb0fe5548271967f1a67130b7105cd6a828e03909a67962e0ea1f61deb649f6bc3f4cef38");
    BOOST_CHECK(IsStandardTx(t, reason));

    /**-5-1041-byte TX_NULL_DATA (non-standard)
    ...
    /**-5-10TX_NULL_DATA w/o PUSHDATA
    t.vout.resize(1);
    t.vout[0].scriptPubKey = CScript() << OP_RETURN;
    BOOST_CHECK(IsStandardTx(t, reason));

    /**-5-10Only one TX_NULL_DATA permitted in all cases
    t.vout.resize(2);
    t.vout[0].scriptPubKey = CScript() << OP_RETURN << ParseHex("04678afdb0fe5548271967f1a67130b7105cd6a828e03909a67962e0ea1f61deb649f6bc3f4cef38");
