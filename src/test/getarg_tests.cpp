...
static void ResetArgs(const std::string& strArg)
{
    ...
    /**-5-10Insert dummy executable name:
    vecArg.insert(vecArg.begin(), "testticoin");
...
BOOST_AUTO_TEST_CASE(boolarg)
{
    ...
    /**-5-10New 0.6 feature: auto-map -nosomething to !-something:
    ResetArgs("-nofoo");
    BOOST_CHECK(!GetBoolArg("-foo", false));
    BOOST_CHECK(!GetBoolArg("-foo", true));
    ...
    ResetArgs("-foo -nofoo");  /**-5-10-foo should win
    BOOST_CHECK(GetBoolArg("-foo", false));
    BOOST_CHECK(GetBoolArg("-foo", true));

    ResetArgs("-foo=1 -nofoo=1");  /**-5-10-foo should win
    BOOST_CHECK(GetBoolArg("-foo", false));
    BOOST_CHECK(GetBoolArg("-foo", true));

    ResetArgs("-foo=0 -nofoo=0");  /**-5-10-foo should win
    BOOST_CHECK(!GetBoolArg("-foo", false));
    BOOST_CHECK(!GetBoolArg("-foo", true));

    /**-5-10New 0.6 feature: treat -- same as -:
    ResetArgs("--foo=1");
    BOOST_CHECK(GetBoolArg("-foo", false));
    BOOST_CHECK(GetBoolArg("-foo", true));
...
BOOST_AUTO_TEST_CASE(boolargno)
{
    ...
    ResetArgs("-nofoo -foo"); /**-5-10foo always wins:
    BOOST_CHECK(GetBoolArg("-foo", true));
    BOOST_CHECK(GetBoolArg("-foo", false));
}
