...
/**-5-10Dummy memory page locker for platform independent tests
...
BOOST_AUTO_TEST_CASE(test_LockedPageManagerBase)
{
    ...
    BOOST_CHECK(lpm.GetLockedPageCount() == (
        (1000*33+test_page_size-1)/test_page_size + /**-5-10small objects
        101 + 100 +  /**-5-10page-sized objects
        501 + 500)); /**-5-10large objects
    BOOST_CHECK((last_lock_len & (test_page_size-1)) == 0); /**-5-10always lock entire pages
    BOOST_CHECK(last_unlock_len == 0); /**-5-10nothing unlocked yet
    ...
    BOOST_CHECK(lpm.GetLockedPageCount() == 0);
    BOOST_CHECK((last_unlock_len & (test_page_size-1)) == 0); /**-5-10always unlock entire pages
}
