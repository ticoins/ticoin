/**-5-10Copyright (c) 2011-2013 The ticoin Core developers
...
ParseScript(string s)
{
    ...
    if (mapOpNames.size() == 0)
    {
        ...
        {
            /**-5-10Allow OP_RESERVED to get into mapOpNames
            ...
            /**-5-10Convenience: OP_ADD and just ADD are both recognized:
    ...
    BOOST_FOREACH(string w, words)
    {
        ...
        {
            /**-5-10Raw hex data, inserted NOT pushed onto stack:
            std::vector<unsigned char> raw = ParseHex(string(w.begin()+2, w.end()));
            result.insert(result.end(), raw.begin(), raw.end());
        }
        else if (w.size() >= 2 && starts_with(w, "'") && ends_with(w, "'"))
        {
            /**-5-10Single-quoted string, pushed as data. NOTE: this is poor-man's
            /**-5-10parsing, spaces/tabs/newlines in single-quoted strings won't work.
            std::vector<unsigned char> value(w.begin()+1, w.end()-1);
            result << value;
        }
        else if (mapOpNames.count(w))
        {
            /**-5-10opcode, e.g. OP_ADD or ADD:
            result << mapOpNames[w];
        }
        else
        {
...
{
    /**-5-10Read tests from test/data/script_valid.json
    /**-5-10Format is an array of arrays
    /**-5-10Inner arrays are [ "scriptSig", "scriptPubKey" ]
    /**-5-10... where scriptSig and scriptPubKey are stringified
    /**-5-10scripts.
    ...
    BOOST_FOREACH(Value& tv, tests)
    {
        ...
        if (test.size() < 2) /**-5-10Allow size > 2; extra stuff ignored (useful for comments)
        {
...
{
    /**-5-10Scripts that should evaluate as invalid
...
{
    /**-5-10Check that PUSHDATA1, PUSHDATA2, and PUSHDATA4 create the same value on
    /**-5-10the stack as the 1-75 opcodes do.
...
sign_multisig(CScript scriptPubKey, std::vector<CKey> keys, CTransaction transaction)
{
    ...
    CScript result;
    //
    /**-5-10NOTE: CHECKMULTISIG has an unfortunate bug; it requires
    /**-5-10one extra item on the stack, before the signatures.
    /**-5-10Putting OP_0 on the stack is the workaround;
    /**-5-10fixing the bug would mean splitting the block chain (old
    /**-5-10clients would not accept new CHECKMULTISIG transactions,
    /**-5-10and vice-versa)
    //
    result << OP_0;
...
BOOST_AUTO_TEST_CASE(script_CHECKMULTISIG23)
{
    ...
    keys.clear();
    keys.push_back(key2); keys.push_back(key2); /**-5-10Can't re-use sig
    CScript badsig1 = sign_multisig(scriptPubKey23, keys, txTo23);
    BOOST_CHECK(!VerifyScript(badsig1, scriptPubKey23, txTo23, 0, flags, 0));

    keys.clear();
    keys.push_back(key2); keys.push_back(key1); /**-5-10sigs must be in correct order
    CScript badsig2 = sign_multisig(scriptPubKey23, keys, txTo23);
    BOOST_CHECK(!VerifyScript(badsig2, scriptPubKey23, txTo23, 0, flags, 0));

    keys.clear();
    keys.push_back(key3); keys.push_back(key2); /**-5-10sigs must be in correct order
    CScript badsig3 = sign_multisig(scriptPubKey23, keys, txTo23);
    BOOST_CHECK(!VerifyScript(badsig3, scriptPubKey23, txTo23, 0, flags, 0));

    keys.clear();
    keys.push_back(key4); keys.push_back(key2); /**-5-10sigs must match pubkeys
    CScript badsig4 = sign_multisig(scriptPubKey23, keys, txTo23);
    BOOST_CHECK(!VerifyScript(badsig4, scriptPubKey23, txTo23, 0, flags, 0));

    keys.clear();
    keys.push_back(key1); keys.push_back(key4); /**-5-10sigs must match pubkeys
    CScript badsig5 = sign_multisig(scriptPubKey23, keys, txTo23);
    BOOST_CHECK(!VerifyScript(badsig5, scriptPubKey23, txTo23, 0, flags, 0));

    keys.clear(); /**-5-10Must have signatures
...
{
    /**-5-10Test the CombineSignatures function
    ...
    /**-5-10Single signature case:
    SignSignature(keystore, txFrom, txTo, 0); /**-5-10changes scriptSig
    combined = CombineSignatures(scriptPubKey, txTo, 0, scriptSig, empty);
    BOOST_CHECK(combined == scriptSig);
    combined = CombineSignatures(scriptPubKey, txTo, 0, empty, scriptSig);
    BOOST_CHECK(combined == scriptSig);
    CScript scriptSigCopy = scriptSig;
    /**-5-10Signing again will give a different, valid signature:
    SignSignature(keystore, txFrom, txTo, 0);
    combined = CombineSignatures(scriptPubKey, txTo, 0, scriptSigCopy, scriptSig);
    BOOST_CHECK(combined == scriptSigCopy || combined == scriptSig);

    /**-5-10P2SH, single-signature case:
    ...
    combined = CombineSignatures(scriptPubKey, txTo, 0, scriptSigCopy, scriptSig);
    BOOST_CHECK(combined == scriptSigCopy || combined == scriptSig);
    /**-5-10dummy scriptSigCopy with placeholder, should always choose non-placeholder:
    ...
    /**-5-10Hardest case:  Multisig 2-of-3
    ...
    /**-5-10A couple of partially-signed versions:
    ...
    /**-5-10Not fussy about order (or even existence) of placeholders or signatures:
