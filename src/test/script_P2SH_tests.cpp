...
/**-5-10Test routines internal to script.cpp:
...
{
    /**-5-10Create dummy to/from transactions:
    CTransaction txFrom;
...
{
    LOCK(cs_main);
    /**-5-10Pay-to-script-hash looks like this:
    /**-5-10scriptSig:    <sig> <sig...> <serialized_script>
    /**-5-10scriptPubKey: HASH160 <hash> EQUAL

    /**-5-10Test SignSignature() (and therefore the version of Solver() that signs transactions)
    ...
    /**-5-108 Scripts: checking all combinations of
    /**-5-10different keys, straight/P2SH, pubkey/pubkeyhash
    ...
    CTransaction txFrom;  /**-5-10Funding transaction:
    string reason;
    ...
    CTransaction txTo[8]; /**-5-10Spending transactions
    for (int i = 0; i < 8; i++)
    {
        txTo[i].vin.resize(1);
    ...
    /**-5-10All of the above should be OK, and the txTos have valid signatures
    /**-5-10Check to make sure signature verification fails if we use the wrong ScriptSig:
    for (int i = 0; i < 8; i++)
        for (int j = 0; j < 8; j++)
        {
...
{
    /**-5-10Make sure only the outer pay-to-script-hash does the
    /**-5-10extra-validation thing:
    ...
    /**-5-10Should not verify, because it will try to execute OP_INVALIDOPCODE
    BOOST_CHECK(!Verify(scriptSig, p2sh, true));

    /**-5-10Try to recur, and verification should succeed because
    /**-5-10the inner HASH160 <> EQUAL should only check the hash:
    CScript p2sh2;
...
{
    LOCK(cs_main);
    /**-5-10Test the CScript::Set* methods
    CBasicKeyStore keystore;
    CKey key[4];
    ...
    CTransaction txTo[4]; /**-5-10Spending transactions
    for (int i = 0; i < 4; i++)
    {
        txTo[i].vin.resize(1);
        txTo[i].vout.resize(1);
...
{
    /**-5-10Test CScript::IsPayToScriptHash()
    uint160 dummy;
    CScript p2sh;
    p2sh << OP_HASH160 << dummy << OP_EQUAL;
    BOOST_CHECK(p2sh.IsPayToScriptHash());

    /**-5-10Not considered pay-to-script-hash if using one of the OP_PUSHDATA opcodes:
...
BOOST_AUTO_TEST_CASE(switchover)
{
    ...
    /**-5-10Validation should succeed under old rules (hash is correct):
...
BOOST_AUTO_TEST_CASE(AreInputsStandard)
{
    ...
    /**-5-10Last three non-standard:
    CScript empty;
    keystore.AddCScript(empty);
    txFrom.vout[3].scriptPubKey = empty;
    txFrom.vout[3].nValue = 4000;
    /**-5-10Can't use SetPayToScriptHash, it checks for the empty Script. So:
    ...
    /**-5-10Make sure adding crap to the scriptSigs makes them non-standard:
    for (int i = 0; i < 3; i++)
    {
