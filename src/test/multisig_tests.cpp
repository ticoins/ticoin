...
sign_multisig(CScript scriptPubKey, vector<CKey> keys, CTransaction transaction, int whichIn)
{
    ...
    CScript result;
    result << OP_0; /**-5-10CHECKMULTISIG bug workaround
...
BOOST_AUTO_TEST_CASE(multisig_verify)
{
    ...
    CTransaction txTo[3]; /**-5-10Spending transaction
    for (int i = 0; i < 3; i++)
    {
        txTo[i].vin.resize(1);
    ...
    /**-5-10Test a AND b:
    keys.clear();
    keys += key[0],key[1]; /**-5-10magic operator+= from boost.assign
...
{
    /**-5-10Tests Solver() that returns lists of keys that are
    /**-5-10required to satisfy a ScriptPubKey
    //
    /**-5-10Also tests IsMine() and ExtractAddress()
    //
    /**-5-10Note: ExtractAddress for the multisignature transactions
    /**-5-10always returns false for this release, even if you have
    /**-5-10one key that would satisfy an (a|b) or 2-of-3 keys needed
    /**-5-10to spend an escrow transaction.
    //
