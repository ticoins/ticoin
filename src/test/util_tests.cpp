...
BOOST_AUTO_TEST_CASE(util_MedianFilter)
{
    ...
    filter.input(20); /**-5-10[15 20]
    BOOST_CHECK_EQUAL(filter.median(), 17);

    filter.input(30); /**-5-10[15 20 30]
    BOOST_CHECK_EQUAL(filter.median(), 20);

    filter.input(3); /**-5-10[3 15 20 30]
    BOOST_CHECK_EQUAL(filter.median(), 17);

    filter.input(7); /**-5-10[3 7 15 20 30]
    BOOST_CHECK_EQUAL(filter.median(), 15);

    filter.input(18); /**-5-10[3 7 18 20 30]
    BOOST_CHECK_EQUAL(filter.median(), 18);

    filter.input(0); /**-5-10[0 3 7 18 30]
...
BOOST_AUTO_TEST_CASE(util_ParseHex)
{
    ...
    /**-5-10Basic test vector
    result = ParseHex("04678afdb0fe5548271967f1a67130b7105cd6a828e03909a67962e0ea1f61deb649f6bc3f4cef38c4f35504e51ec112de5c384df7ba0b8d578a4c702b6bf11d5f");
    BOOST_CHECK_EQUAL_COLLECTIONS(result.begin(), result.end(), expected.begin(), expected.end());

    /**-5-10Spaces between bytes must be supported
    result = ParseHex("12 34 56 78");
    BOOST_CHECK(result.size() == 4 && result[0] == 0x12 && result[1] == 0x34 && result[2] == 0x56 && result[3] == 0x78);

    /**-5-10Stop parsing at invalid value
...
BOOST_AUTO_TEST_CASE(util_ParseParameters)
{
    ...
    /**-5-10expectation: -ignored is ignored (program name argument),
    /**-5-10-a, -b and -ccc end up in map, -d ignored because it is after
    /**-5-10a non-option argument (non-GNU option parsing)
...
BOOST_AUTO_TEST_CASE(util_GetArg)
{
    ...
    /**-5-10strtest2 undefined on purpose
    mapArgs["inttest1"] = "12345";
    mapArgs["inttest2"] = "81985529216486895";
    /**-5-10inttest3 undefined on purpose
    mapArgs["booltest1"] = "";
    /**-5-10booltest2 undefined on purpose
    mapArgs["booltest3"] = "0";
    mapArgs["booltest4"] = "1";
...
BOOST_AUTO_TEST_CASE(util_ParseMoney)
{
    ...
    /**-5-10Attempted 63 bit overflow should fail
...
BOOST_AUTO_TEST_CASE(util_seed_insecure_rand)
{
    ...
    {
        int mask = 1;
        /**-5-10Really rough binomal confidence approximation.
