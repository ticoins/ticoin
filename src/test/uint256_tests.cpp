...
const double R1Ldouble = 0.4887374590559308955; /**-5-10R1L equals roughly R1Ldouble * 2^256
...
BOOST_AUTO_TEST_CASE( basics ) /**-5-10constructors, equality, inequality
{
    BOOST_CHECK(1 == 0+1);
    /**-5-10constructor uint256(vector<char>):
    ...
    /**-5-10String Constructor and Copy Constructor
...
BOOST_AUTO_TEST_CASE( shifts ) { /**-5-10"<<"  ">>"  "<<="  ">>="
...
BOOST_AUTO_TEST_CASE( unaryOperators ) /**-5-10!    ~    -
{
...
/**-5-10Check if doing _A_ _OP_ _B_ results in the same as applying _OP_ onto each
/**-5-10element of Aarray and Barray, and then converting the result into a uint256.
...
BOOST_AUTO_TEST_CASE( comparison ) /**-5-10<= >= < >
{
    uint256 TmpL;
    for (unsigned int i = 0; i < 256; ++i) {
        TmpL= OneL<< i;
...
BOOST_AUTO_TEST_CASE( methods ) /**-5-10GetHex SetHex begin() end() size() GetLow64 GetSerializeSize, Serialize, Unserialize
{
    ...
    for (int i = 53; i > 0; --i) /**-5-10doubles can store all integers in {0,...,2^54-1} exactly
    {
...
BOOST_AUTO_TEST_CASE( getmaxcoverage ) /**-5-10some more tests just to get 100% coverage
{
    /**-5-10~R1L give a base_uint<256>
