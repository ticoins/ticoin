...
#include "chainparams.h" /**-5-10for Params().RPCPort()
...
Object CallRPC(const string& strMethod, const Array& params)
{
    ...
    bool fWait = GetBoolArg("-rpcwait", false); /**-5-10-rpcwait means try until server has started
    do {
    ...
    /**-5-10Receive HTTP reply status
    int nProto = 0;
    int nStatus = ReadHTTPStatus(stream, nProto);

    /**-5-10Receive HTTP reply message headers and body
...
void ConvertTo(Value& value, bool fAllowNull=false)
{
    ...
    {
        /**-5-10reinterpret string as unquoted json value
        Value value2;
...
/**-5-10Convert strings to command-specific RPC representation
Array RPCConvertValues(const std::string &strMethod, const std::vector<std::string> &strParams)
{
    ...
    //
    /**-5-10Special case non-string parameter types
    //
...
int CommandLineRPC(int argc, char *argv[])
{
    ...
    try
    {
        ...
        /**-5-10Parameters default to strings
...
std::string HelpMessageCli(bool mainProgram)
{
    ...
    if(mainProgram)
    {
        ...
        strUsage += "\n" + _("SSL options: (see the ticoin Wiki for SSL setup instructions)") + "\n";
