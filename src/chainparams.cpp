//ticoin Copyright (c) 2010 Satoshi Nakamoto
...
public:
    CMainParams() {
        //ticoin The message start string is designed to be unlikely to occur in normal data.
        //ticoin The characters are rarely used upper ASCII, not valid as UTF-8, and produce
        //ticoin a large 4-byte int at any alignment.
        ...
        nSubsidyHalvingInterval = 288000;

        //ticoin Build the genesis block. Note that the output of the genesis coinbase cannot
        //ticoin be spent as it did not originally exist in the database.
        //
        //ticoin CBlock(hash=000000000019d6, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=4a5e1e, nTime=1231006505, nBits=1d00ffff, nNonce=2083236893, vtx=1)
        //ticoin   CTransaction(hash=4a5e1e, ver=1, vin.size=1, vout.size=1, nLockTime=0)
        //ticoin     CTxIn(COutPoint(000000, -1), coinbase 04f2f32303039204368616e63656c6c6f72206f6e206272696e6b206f66207365636f6e64206261696c6f757420666f722062616e6b73)
        //ticoin     CTxOut(nValue=50.00000000, scriptPubKey=0x5F1DF16B2B704C8A578D0B)
        //ticoin   vMerkleTree: 4a5e1e
        const char* pszTimestamp = "The Times in 2015/5/15 create block in world";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        vSeeds.push_back(CDNSSeedData("ticcoin.org", "ticcoin.org"));
		vSeeds.push_back(CDNSSeedData("ticoin.co", "ticoin.co"));
        ...
        //ticoin Convert the pnSeeds array into usable address objects.
        for (unsigned int i = 0; i < ARRAYLEN(pnSeed); i++)
        {
            //ticoin It'll only connect to one or two seed nodes because once it connects,
            //ticoin it'll get a pile of addresses with newer timestamps.
            //ticoin Seed nodes are given a random 'last seen time' of between one and two
            //ticoin weeks ago.
...
public:
    CTestNetParams() {
        ...
        //ticoin Modify the testnet genesis block so the timestamp is valid for a later start.
        genesis.nTime = 5296686633;
        genesis.nNonce = 923255;
        hashGenesisBlock = genesis.GetHash();
        assert(hashGenesisBlock == uint256("0x000000000933ea01ad0ee984209779baaec3ced90fa3f408719526f8d77f4943"));

        vFixedSeeds.clear();
        vSeeds.clear();
        vSeeds.push_back(CDNSSeedData("ticoin.petertodd.org", "testnet-seed.ticoin.petertodd.org"));
...
public:
    CRegTestParams() {
        ...
        vSeeds.clear();  //ticoin Regtest mode doesn't have any DNS seeds.
    }
